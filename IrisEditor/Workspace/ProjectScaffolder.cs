...
namespace IrisEditor.Workspace
{
    internal static class ProjectScaffolder
    {
        ...
        public const string EngineOnlyPropsFileName = "Iris.Engine.g.props";
        ...
        {
            string enginePops = ResolveEngineProps(engineProject, out error);
            ...
            try
            {
                WriteIfChanged(Path.Combine(projectRoot, EngineOnlyPropsFileName), enginePops);
                WriteIfChanged(Path.Combine(projectRoot, EnginePropsFileName), RootProps(projectRoot));
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            if (!MatchesOnDisk(path, content))
                File.WriteAllText(path, content);
        }

        private static string RootProps(string projectRoot)
        {
            return $"""
                <Project>

                  <Import Project="{EngineOnlyPropsFileName}" Condition="Exists('{EngineOnlyPropsFileName}')" />

                {ReleaseWindowGroup}

                {AssemblySection(projectRoot)}</Project>
                """;
        }

        private static string AssemblySection(string projectRoot)
        {
            var definitions = AssemblyDefinitions.Generate(projectRoot);
            ...
            foreach (var definition in definitions)
                builder.AppendLine($"    <Compile Remove=\"{definition.RelativeDirectory.Replace('/', '\\')}\\**\\*.cs\" />");
            ...
            foreach (var definition in definitions)
            {
                string relative = Path.GetRelativePath(projectRoot, definition.ProjectPath).Replace('/', '\\');
                builder.AppendLine($"    <ProjectReference Include=\"{relative}\" />");
            }
        ...
        private static string GitIgnoreTemplate()
        {
            return $"""
                ...
                {EnginePropsFileName}
                {EngineOnlyPropsFileName}

                # Generated from .asmdef files every time the project is opened
                *{AssemblyDefinitions.GeneratedProjectSuffix}
